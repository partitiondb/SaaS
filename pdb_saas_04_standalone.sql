...
where dbid in (db_id('SaaSGate'),db_id('AtlanticDB'),db_id('CaribbeanDB'),db_id('MonacoDB'),db_id('SaaSGlobalDB'),db_id('SaaSCommonDB'),db_id('SaaSBillingDB')) and cmd = 'AWAITING COMMAND' and spid <> @@spid;
exec(@sql);
end;
go
...
if db_id('SaaSBillingDB') 	is not null drop database SaaSBillingDB;
...
exec PdbcreatePartition 'SaaSGate','SaaSBillingDB',@DatabaseTypeId=4,@StandAloneSchemaName='Billing';
...
create table Billing.Cashouts
	(	Id					bigint identity(1,1) 	not null primary key
	,	BrandId				PartitionDBType			not null references Brands (Id)
	,	PlayerId			smallint				not null references Players (Id)
	,	BankNumber			nvarchar(16)
	,	BankBranch			nvarchar(32)
	,	BankAccount			nvarchar(64)
	,	CashValue			decimal(18,3)			not null
	,	ChipsCashed			smallint				not null
	);
...
create table Billing.Purchases
	...
	,	BrandId				PartitionDBType			not null references Brands (Id)
	,	PlayerId			smallint				not null references Players (Id)
	,	PurchaseValue		decimal(18,3)			not null
	,	ChipsPurchased		smallint				not null
	);
...
create synonym Billing.Deposits for Billing.Purchases;
go
create synonym Billing.Payments for Billing.Cashouts;
go
...
begin
	...
	begin
		...
		insert into Billing.PdbPurchases (BrandId,PlayerId,PurchaseValue,ChipsPurchased) values (@BrandId,@PlayerId,@PurchaseValue,@ChipsPurchased);
